  ...
  return (
    <div className="pt-14">
      <div className="mx-auto px-2 sm:px-4 lg:px-6 py-8">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900">My Booked Tours</h1>
...
<div className="mb-6 text-right">
  <button
    ...
    className="inline-block px-5 py-2.5 rounded-lg text-white text-base font-medium transition-transform hover:scale-105 cursor-pointer"
                    ...
                    <div key={index} className={`bg-white min-h-[120px] p-2 border-r border-b border-gray-200 ${!isCurrentMonth ? 'bg-gray-50 opacity-50' : ''} ${new Date().toDateString() === date.toDateString() ? 'ring-2 ring-blue-500 z-10' : ''}`}>
                      ...
                      <div className="space-y-1 mt-1">
                        {bookings.slice(0, 2).map(booking => (
                          <div key={booking.id} onClick={() => handleViewDetails(booking)} className={`text-base p-1 rounded cursor-pointer hover:opacity-80 ${getStatusColor(booking.status)}`}>
                            <div className="truncate font-semibold">{booking.tourTitle}</div>
                            <div className="text-gray-700">{booking.startTime}</div>
                          </div>
                        ))}
                        {bookings.length > 2 && <div className="text-base text-center text-gray-500 mt-1">+{bookings.length - 2} more</div>}
                      </div>
                    </div>
                  ))}
                </div>
                  ...
                  <table className="w-full">
                    <thead className="bg-gray-50 border-b border-gray-200">
                      <tr>
                        ...
                        <th className="px-6 py-3 text-left text-base font-medium text-gray-700 uppercase tracking-wider">Location</th>
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {paginatedBookings.map((booking) => (
                        <tr key={booking.id} className={`hover:bg-gray-50 transition-colors ${new Date(booking.date) < new Date() && booking.status !== 'completed' ? 'opacity-60' : ''}`}>
                          ...
                          <td className="px-6 py-4"><div className="text-base text-gray-600"><i className="bi bi-geo-alt text-gray-400 mr-1"></i>{booking.location}</div></td>
                          <td className="px-6 py-4 whitespace-nowrap"><div className="text-base font-medium text-gray-900">{booking.numberOfGuests}</div></td>
                          <td className="px-6 py-4 whitespace-nowrap"><span className={`px-2 py-1 inline-flex text-base leading-5 font-semibold rounded-full ${getStatusColor(booking.status)}`}><i className={`bi ${getStatusIcon(booking.status)} mr-1`}></i>{booking.status}</span></td>
              ...
              <div className="p-6">
                ...
                <div className="flex gap-3">
                  {selectedBooking.status !== 'cancelled' && selectedBooking.status !== 'completed' && (<>
                    <button onClick={() => { setShowDetailModal(false); handleEdit(selectedBooking); }} className="flex-1 px-6 py-3 bg-gray-300 text-gray-700 rounded-lg hover:bg-gray-400 transition-colors font-medium"><i className="bi bi-pencil mr-2"></i>Edit Booking</button>
            ...
            <div className="bg-white rounded-lg max-w-3xl w-full max-h-[90vh] overflow-y-auto shadow-2xl">
              <div className="p-6">
                ...
                <div className="flex gap-3 mt-6">
                  ...
                  <button onClick={() => { setShowAddEditModal(false); setEditingBooking(null); }} className="px-6 py-3 cursor-pointer bg-gray-300 text-black rounded-lg hover:bg-gray-400 font-medium">Cancel</button>
                </div>
              </div>
            </div>
          </div>
