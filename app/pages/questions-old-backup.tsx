...
const AssessmentContent: React.FC = () => {
  ...
  // Check if user should be redirected after assessment
  useEffect(() => {
    if (assessmentResult && assessmentResult.percentage >= 80 && assessmentCompleted) {
      // Wait 3 seconds then redirect
      setTimeout(() => {
        // Redirect agents to their dashboard, others to KYC
  ...
  }, [assessmentResult, assessmentCompleted, user, router]);
  ...
  const saveAssessmentToDatabase = async (results: AssessmentResult) => {
    try {
      ...
      const questionsAndAnswers = results.questionsResults.map((result) => ({
        questionId: parseInt(result.questionId, 10),
        question: result.question,
        answer: Array.isArray(result.userAnswer) ? result.userAnswer.join(', ') : result.userAnswer,
      ...
      // Use the API service method which points to correct endpoint
      await api.submitAssessment(assessmentData);
      console.log('Assessment saved to database successfully');

      // Clear assessment cache so next check fetches fresh data
                ...
                <div>
                  ...
                  <p className="text-green-700 text-sm">
                    {user?.userType === 'agent'
                      ...
                      : 'You will be redirected to complete your KYC verification in a few seconds...'}
                  </p>
                </div>
              </div>
            </div>
