...
import BookingModal, {
  BookingData,
} from "@/app/components/modals/BookingModal";
...
const GuestUnlockHistory = () => {
  ...
  const [activeTab, setActiveTab] = useState<"history" | "deal-codes">(
    "history"
  );
  const [selectedUnlock, setSelectedUnlock] =
    useState<UnlockHistoryEntry | null>(null);
  ...
  const transformUnlockData = (backendUnlock: any): UnlockHistoryEntry => {
    ...
    const isPaymentCompleted = backendUnlock.paymentStatus === "COMPLETED";

    // Parse property images
    let propertyImage = "";
    try {
      const images =
        typeof backendUnlock.property?.images === "string"
    ...
    // Parse address string to PropertyAddress object - use property.location
    ...
    const mapPaymentMethod = (
      method: string
    ): "non_refundable" | "monthly_booking" | "deal_code" => {
    ...
    // Determine amount paid and currency from new API format
    const amountPaid = backendUnlock.paymentAmountRwf || backendUnlock.amountPaid || 0;
    const currency = "RWF";

    return {
      id: backendUnlock.unlockId || backendUnlock.id,
      propertyId: String(backendUnlock.property?.id || backendUnlock.propertyId),
      propertyTitle: backendUnlock.property?.name || "Property",
      propertyImage: propertyImage || "/placeholder-property.jpg",
      unlockDate: backendUnlock.unlockedAt || backendUnlock.createdAt || backendUnlock.unlockDate,
      paymentMethod: mapPaymentMethod(backendUnlock.paymentMethod),
      amountPaid: amountPaid,
      currency: currency,
      address: parseAddress(backendUnlock.property?.location || backendUnlock.address),
      hostContact: {
        name:
          backendUnlock.hostContactInfo?.hostName ||
          backendUnlock.hostContact?.name ||
          "Host",
        phone:
          backendUnlock.hostContactInfo?.hostPhone ||
          backendUnlock.hostContact?.phone ||
          "",
        email:
          backendUnlock.hostContactInfo?.hostEmail ||
          backendUnlock.hostContact?.email ||
          "",
        whatsapp:
          backendUnlock.hostContactInfo?.whatsapp ||
          backendUnlock.hostContact?.whatsapp,
        preferredContact: (backendUnlock.hostContactInfo
          ?.preferredContactMethod ||
          backendUnlock.hostContact?.preferredContact ||
          "email") as "phone" | "email" | "whatsapp",
        profileImage:
          backendUnlock.hostContactInfo?.hostProfileImage ||
          backendUnlock.hostContact?.profileImage,
        verified:
          backendUnlock.hostContactInfo?.verified ||
          backendUnlock.hostContact?.verified ||
          false,
      },
      appreciationSubmitted: backendUnlock.appreciationSubmitted || false,
      appreciationLevel: backendUnlock.appreciationLevel,
      status: backendUnlock.status || (isPaymentCompleted ? "unlocked" : "pending"),
      canCancel:
        isPaymentCompleted &&
        (backendUnlock.canCancel ??
          backendUnlock.paymentMethod === "three_month_30_percent"),
      canRequestDealCode:
        isPaymentCompleted &&
        (backendUnlock.canRequestDealCode ??
          (!backendUnlock.appreciationSubmitted &&
            backendUnlock.paymentMethod === "three_month_30_percent")),
      canBook: isPaymentCompleted && (backendUnlock.canBook ?? false),
  ...
  const loadUnlockData = async () => {
    try {
      ...
      if (unlocksResponse.data && unlocksResponse.data.success) {
        ...
        const transformedUnlocks = (unlocksData.unlocks || []).map(
          transformUnlockData
        );
        ...
        if (
          dealCodesResponse.data &&
          dealCodesResponse.data.success &&
          dealCodesResponse.data.data
        ) {
  ...
  const handleViewOnMap = (unlock: UnlockHistoryEntry) => {
    if (
      !unlock.address ||
      (!unlock.address.latitude && !unlock.address.longitude)
    ) {
      setNotification({
  ...
  // Unlock Address Handler - Redirect to monthly rentals page
  const handleUnlockAddress = (unlock: UnlockHistoryEntry) => {
    const encodedId = encodeId(unlock.propertyId);
    const url = `https://jambolush.com/monthly-rentals/${encodedId}`;
  ...
  const handleSubmitDealCodeRequest = async (
    ...
    try {
      ...
      if (response.data && response.data.success) {
        ...
        if (response.data.data) {
          const dealCode = response.data.data.dealCodeGenerated
            ? response.data.data.dealCode?.code
            : "";
        ...
        setNotification({
          message:
            ...
            "Feedback received but no deal code generated",
          type: "warning",
        });
        return "";
      }
  ...
  return (
    <div className="min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        >
          <div className="bg-white rounded-2xl shadow-sm hover:shadow-lg transition-all duration-300 p-6 shadow-sm shadow-gray-400/70">
        ...
        <motion.div
          ...
          className="mb-6"
        >
          <div className="bg-white rounded-2xl shadow-sm p-2 inline-flex shadow-sm shadow-gray-400/70">
            <button
              ...
              {stats?.recentUnlocks && stats.recentUnlocks.length > 0 ? (
                stats.recentUnlocks.map((unlock, index) => (
                  <motion.div
                    ...
                    className="bg-white rounded-2xl shadow-gray-400/70 shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden group"
                  >
                    <div className="p-6 shadow-sm shadow-gray-400/70">
                      <div className="flex flex-col lg:flex-row lg:items-center gap-6">
                        {/* Property Image */}
                        <div className="relative w-full lg:w-48 h-48 lg:h-32 rounded-xl overflow-hidden flex-shrink-0">
                          <img
                            src={
                              unlock.propertyImage ||
                          ...
                          <div className="absolute top-3 right-3 px-3 py-1 bg-white/90 backdrop-blur-sm rounded-full text-xs font-semibold text-gray-900">
                            {unlock.paymentMethod === "non_refundable"
                              ? "Non-refundable"
                              : unlock.paymentMethod === "monthly_booking"
                              ? "30% Booking"
                              : "Deal Code"}
                          </div>
                        </div>
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex flex-wrap gap-4 text-sm text-gray-600 mb-3">
                            <span className="flex items-center">
                              ...
                              {unlock.address?.city || "Unknown"},{" "}
                              {unlock.address?.country || "Rwanda"}
                            </span>
                            <span className="flex items-center">
                          ...
                          {unlock.address && (
                            <div className="bg-gray-50 rounded-xl p-3 mb-3">
                              <p className="text-sm text-gray-700">
                                ...
                                {unlock.address.street &&
                                  `${unlock.address.street}, `}
                                {unlock.address.city &&
                                  `${unlock.address.city}, `}
                                {unlock.address.state &&
                                  `${unlock.address.state} `}
                          ...
                          {/* Host Contact - Only show if payment is completed */}
                          {unlock.status !== "pending" && (
                            <div className="flex flex-wrap items-center gap-3">
                              ...
                              <div className="flex gap-2">
                                <button
                                  onClick={() =>
                                    handleContactHost(unlock, "phone")
                                  }
                                ...
                                <button
                                  onClick={() =>
                                    handleContactHost(unlock, "email")
                                  }
                                ...
                                {unlock.hostContact.whatsapp && (
                                  <button
                                    onClick={() =>
                                      handleContactHost(unlock, "whatsapp")
                                    }
                                    className="px-3 py-1.5 bg-green-50 hover:bg-green-100 text-green-700 rounded-lg text-sm font-medium transition-colors"
                                  >
                          ...
                          {unlock.rewardReceived && (
                            <div className="mt-3 inline-flex items-center px-3 py-1.5 bg-gradient-to-r from-amber-100 to-yellow-100 rounded-lg">
                              ...
                              <span className="text-xs font-medium text-amber-800">
                                {unlock.rewardReceived.type === "both"
                                  ? "Refund + Deal Code Received"
                                  : unlock.rewardReceived.type === "deal_code"
                                  ? "Deal Code Received"
                                  : "Refund Processed"}
                              </span>
                            </div>
                          ...
                          {/* Status Badge */}
                          {unlock.status === "pending" && (
                            <div className="mt-3 inline-flex items-center px-3 py-1.5 bg-yellow-100 rounded-lg">
                              <i className="bi bi-clock-fill text-yellow-600 mr-2"></i>
                        ...
                        <div className="flex-shrink-0">
                          <div className="flex flex-col gap-2 w-full lg:w-auto">
                            {/* Unlock Address Button - Show for pending payments */}
                            {unlock.status === "pending" && (
                              <button
                                onClick={() => handleUnlockAddress(unlock)}
                                className="px-4 py-2 bg-gradient-to-r from-[#F20C8F] to-rose-400 text-white text-sm font-medium rounded-lg shadow-md hover:shadow-lg transition-all duration-300"
                              >
                                <i className="bi bi-unlock mr-1.5"></i>
                                Unlock Address
                              </button>
                            )}

                            {/* View Map Button - Only show if payment is completed */}
                            {unlock.status !== "pending" && (
                              <button
                            ...
                            {/* Cancel Request Button - Show only if payment completed and 30% paid and not cancelled/completed */}
                            {unlock.canCancel &&
                              unlock.status !== "cancelled" &&
                              unlock.status !== "pending" &&
                              !unlock.bookingCompleted && (
                            ...
                            {unlock.canBook &&
                              unlock.appreciationLevel === "appreciated" &&
                              !unlock.bookingCompleted && (
                                <button
                ...
                >
                  ...
                  <p className="text-gray-600 mb-6">
                    Start exploring properties and unlock addresses to view host
                    contact information
                  </p>
                  <button
                    onClick={() => (window.location.href = "https://jambolush.com/moonthly-rentals")}
            ...
            >
              {stats?.dealCodes && stats.dealCodes.length > 0 ? (
                stats.dealCodes.map((code, index) => {
                  ...
                  const isActive =
                    code.isActive && !isExpired && code.remainingUnlocks > 0;
                    ...
                    >
                      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                        ...
                        {isActive && (
                          <div className="flex-shrink-0">
                            <button
                              onClick={() => (window.location.href = "https://jambolush.com/monthly-rentals")}
