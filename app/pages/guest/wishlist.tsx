...
const WishlistPage: React.FC = () => {
    ...
    const fetchWishlist = async () => {
      ...
      try {
        ...
        if (response.data.success) {
          const mappedData = response.data.data.items.map(
            (item: any): WishlistItem => ({
              ...
              image:
                item.itemDetails.image || "https://via.placeholder.com/400x300",
  ...
  const summaryStats = useMemo(() => {
    return {
      ...
      available: wishlistItems.filter((item) => item.status === "available")
        .length,
      unavailable: wishlistItems.filter((item) => item.status === "unavailable")
        .length,
      priceDropped: wishlistItems.filter(
        (item) => item.status === "price-dropped"
      ).length,
  ...
  const handleMoveToBookings = (item: WishlistItem) => {
    ...
    const encodedId = encodeId(item.propertyId);
    const baseUrl = item.itemType === "tour" ? "tours" : "spaces";
    const url = `https://jambolush.com/${baseUrl}/${encodedId}?token=${token}`;
              ...
              <div className="relative aspect-square mb-3 overflow-hidden rounded-xl">
                ...
                {item.status !== "available" && (
                  <div className="absolute top-3 left-3">
                    <span
                      className={`px-2.5 py-1 text-xs font-medium rounded-full border backdrop-blur-sm bg-white/90 ${getStatusColor(
                        item.status
                      )}`}
                    >
            ...
            <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto">
              ...
              <div className="p-6">
                <div className="flex justify-between items-start mb-4">
                  ...
                  >
                    {selectedItem.status === "price-dropped"
                      ? "Price drop"
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {selectedItem.userRating && (
                      <>
                        <span className="text-gray-600">·</span>
                        <span className="text-gray-600">
                          You rated: {selectedItem.userRating}/5
                        </span>
                      </>
                    )}
                  </div>
